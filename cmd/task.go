...
func init() {
	...
	fs.StringVar(&taskLanguages, "languages", "", "List of languages in the task. One of `users` or `groups` must be provided (JSON, required).")
	_ = taskCreateCmd.MarkFlagRequired("languages")
	fs.BoolVar(&autoCloseLang, "auto-close-languages", true, "Whether languages should be closed automatically upon completion of the last item.")
	fs.BoolVar(&autoCloseTask, "auto-close-task", true, "Whether the task should be automatically closed upon all language completion.")
	...
	fs.StringVar(&taskLanguages, "languages", "", "List of languages to update. JSON, differs from a creation list. See https://lokalise.com/api2docs/curl/#transition-update-a-task-put for details")
	fs.BoolVar(&autoCloseLang, "auto-close-languages", true, "Whether languages should be closed automatically upon completion of the last item.")
	fs.BoolVar(&autoCloseTask, "auto-close-task", true, "Whether the task should be automatically closed upon all language completion.")
